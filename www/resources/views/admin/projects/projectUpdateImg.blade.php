@extends('layouts.app', ['title' => __('admin.update_project_img')])
...
@section('content')
    ...
    <div class="header pb-8 pt-5 d-flex align-items-center" style="background-image: url(../argon/img/theme/instagram-1.jpg); background-size: cover; background-position: center top;">
        ...
        <div class="container-fluid d-flex align-items-center">
            <div class="row">
                <div class="col-md-12">
                    <h1 class="text-white">{{__('admin.update_project_img')}}</h1>
                </div>
            </div>
        </div>
    </div>
    <form method="post" action="{{ route(\App\Http\Controllers\Api\ProjectsController::ROUTE_PROJECT_UPDATE_IMG, $id) }}" autocomplete="off" enctype="multipart/form-data">
                                    ...
                                    <div class="card-body pt-0 pt-md-4">
                                        <div class="row align-items-center">
                                            <div class="col-12 card-profile-pdf-image pl-0">
                                                @foreach($arProjectsImg as $img)
                                                    <div id="img_container">
                                                        <img class="img-responsive" width="150" height="150" src="{{ asset('storage') .'/'.  $img['img'] }}"/>
                                                        <a href="{{ route(\App\Http\Controllers\Api\ProjectsController::ROUTE_PROJECT_IMG_DELETE , $img['id'])}}" class="button btn-danger"><i class="fas fa-times"></i></a>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
