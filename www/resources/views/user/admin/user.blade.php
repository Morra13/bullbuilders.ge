                    ...
                    <div class="card-header text-center border-0 pt-8 pt-md-4 pb-0 pb-md-4">
                        <div class="d-flex justify-content-between">
                            ...
                            @if ($user->instruction_count)
                                <a href="{{route(\App\Http\Controllers\InstructionController::ROUTE_ADMIN_LIST)}}?user_id={{$user->id}}" class="btn btn-sm btn-default float-right">{{ __('Instructions') }}</a>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="card-profile-stats d-flex justify-content-center mt-md-5">
                                    <div>
                                        <span class="heading text-nowrap">{{ $user->instruction_count }}</span>
                                        <span class="description">{{ __('Instructions') }}</span>
                                    </div>
                                    <div>
                                        <span class="heading text-nowrap">{{ $user->balance_view }}</span>
                                        <span class="description">{{ __('Balance') }}</span>
                                    </div>
                                    <div>
                                        <span class="heading text-nowrap">{{ $user->sales_view }}</span>
                                        <span class="description">{{ __('Sales') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
