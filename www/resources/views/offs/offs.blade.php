@extends('layouts.app', ['title' => __('ჩამოწერა')])
...
@section('content')
    <div class="header pb-3 pt-7 d-flex align-items-center" style="background-image: url(../argon/img/theme/instagram-2.jpg); background-size: cover; background-position: center top;">
        ...
        <div class="container-fluid d-flex align-items-center">
            <div class="row">
                <div class="col-md-12">
                    <p class="text-white mt-0 mb-7">ჩამოწერა</p>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <div class="card">
                            ...
                            <div class="table-responsive">
                                <table class="table align-items-center table-flush">
                                    ...
                                    <tbody class="list">
                                    @foreach($arOffs as $key => $offs)
                                        @include('offs.row-offs', [
                                            'offs'      => $offs,
                                            'key'       => $key,
                                            ]
                                        )
            ...
            <div class="col-xl-3 order-xl-1 mb-5 mb-xl-2">
                <div class="card card-profile shadow">
                    <div class="card-body pt-0 pt-md-4">
                        <form method="post" action="{{ route(\App\Http\Controllers\Api\OffsController::ROUTE_ADD_IN_OFFS) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('post')
                        ...
                        <?
                        $bool = false;
                        foreach ($arOffs as $value) {
                            $bool = (!empty($value));
                        }
                        if ($bool) {
                        ?>
                        <form method="post" action="{{ route(\App\Http\Controllers\Api\OffsController::ROUTE_CREATE_OFFS) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('post')
                            <div class="pl-lg-4">
                                <div class="input-group mb-3">
