...
?>
<tr style="{{ $style }}">
    <form method="post" action="{{ route(\App\Http\Controllers\Api\OffsController::ROUTE_UPDATE_OFFS) }}" autocomplete="off" enctype="multipart/form-data">
        @csrf
        @method('post')
        ...
        <td class="budget col-xl-1">
            <div class="input-group mb-3">
                <span class="input-group-text badge-success" >{{ $offs->qty }}</span>
                <input type="text" class="form-control min-vh-100" aria-label="" name="qty" id="qty" value="{{ $offs->qty }}" placeholder="{{ $offs->qty }}">
        ...
        <td class="budget col-xl-1">
            <div class="input-group mb-3">
                <input type="hidden" name="id" value="{{ $offs->id }}">
